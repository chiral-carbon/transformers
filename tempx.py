...
if __name__ == "__main__":
    ...
    tests_to_run = [x for x in test_list_items if "/test_modeling_" in x][:8]
    # L = sorted([x for x in test_list_items if "/test_modeling_" not in x and "/test_tokenization_" in x])

    # tests_to_run = " ".join(tests_to_run)

    pytest.main(["-n", "3", "--forked", "--max-worker-restart", "0", "--dist", "loadfile", "-s", "--make-reports", "tests_torch"] + tests_to_run)
